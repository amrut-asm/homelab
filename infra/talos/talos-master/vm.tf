resource "libvirt_volume" "root_volume" {
  name   = "talos-master-root${count.index+1}"
  source = "images/talos.qcow2"
  count = length(var.talos_master_ip_list)
}
...
data "template_file" "network_config" {
  ...
  {
    address = element(var.talos_master_ip_list, count.index),
...
resource "libvirt_cloudinit_disk" "cloud-init" {
  name           = "cloud-init-talos-master${count.index+1}.iso"
...
resource "libvirt_domain" "talos_server" {
  ...
  network_interface {
    bridge = "switch0"
    mac = element(var.talos_master_mac_list, count.index)
  }
